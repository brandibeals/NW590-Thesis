...
## Load models
SAE_loaded = load_model('Models/SAE_train_5timestep_20210220.h5', compile = False)
SAE_loaded.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
print(SAE_loaded.summary())

encoder_loaded = load_model('Models/Encoder_train_5timestep_20210220.h5', compile = False)
encoder_loaded.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
print(encoder_loaded.summary())
...
loss, acc = SAE_loaded.evaluate(x_validation_loaded, x_validation_loaded)
print('Restored model accuracy: {:5.2f}%'.format(100 * acc))

# Generate encoded predictions
SAE_predictions_val = encoder_loaded.predict(x_validation_loaded)
SAE_predictions_val = SAE_predictions_val.reshape((x_validation_loaded.shape[0], 
                                                   x_validation_loaded.shape[1], 
                                                   SAE_predictions_val.shape[1] // x_validation_loaded.shape[1]))

# Save predictions
np.savez_compressed(r'..\Data\SAE_train_encoded_5timestep_validation.npz', SAE_predictions_val)
...
# Calculate accuracy score
loss, acc = SAE_loaded.evaluate(x_test_loaded, x_test_loaded)
print('Restored model accuracy: {:5.2f}%'.format(100 * acc))

# Generate encoded predictions
SAE_predictions_test = encoder_loaded.predict(x_test_loaded)
SAE_predictions_test = SAE_predictions_test.reshape((x_test_loaded.shape[0], 
                                                   x_test_loaded.shape[1], 
                                                   SAE_predictions_test.shape[1] // x_test_loaded.shape[1]))

# Save predictions
np.savez_compressed(r'..\Data\SAE_train_encoded_5timestep_test.npz', SAE_predictions_test)
...
batch_size = 32
epochs = 100
obs = x_validation_loaded.shape[0]
n_steps = x_validation_loaded.shape[1]
input_dim = x_validation_loaded.shape[2]
encoding_dim = n_steps*n_features
...
SAE.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
encoder.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
SAE_history = SAE.fit(x_validation_loaded, x_validation_loaded, epochs=epochs, batch_size=batch_size)

# Save model
SAE.save('Models/SAE_validation_5timestep_%s.h5' % today)
encoder.save('Models/Encoder_validation_5timestep_%s.h5' % today)
...
epochs_range = range(1, epochs+1)
SAE_loss = SAE_history.history['loss']
plt.plot(epochs_range, SAE_loss, label='Validation loss', color='tab:blue')
plt.title('Validation loss for SAE')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig('..\Images\SAE_validation_loss_5timestep_%s.png' % today, bbox_inches='tight', dpi=300)
plt.show()
...
plt.figure(figsize=(10, 6))
SAE_acc = SAE_history.history['accuracy'] #acc
plt.plot(epochs_range, SAE_acc, label='Validation acc', color='tab:blue')
plt.title('Validation accuracy for SAE')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('..\Images\SAE_validation_accuracy_5timestep_%s.png' % today, bbox_inches='tight', dpi=300)
plt.show()

# Generate encoded predictions
SAE_predictions = encoder.predict(x_validation_loaded)
SAE_predictions = SAE_predictions.reshape((obs, n_steps, n_features))

# Save predictions
np.savez_compressed(r'..\Data\SAE_validation_encoded_5timestep.npz', SAE_predictions)

# Build combined array
ticker_date = validation_dateticker[:,-1,-2:]
SAE_predictions_analyze = SAE_predictions[:,-1,:]
SAE_predictions_analyze = np.column_stack((SAE_predictions_analyze, ticker_date))

# Save predictions for analysis
np.savetxt('..\Data\SAE_validation_encoded_5timestep_analyze.csv', SAE_predictions_analyze, delimiter=',', fmt='%s')
...
batch_size = 32
epochs = 100
obs = x_test_loaded.shape[0]
n_steps = x_test_loaded.shape[1]
input_dim = x_test_loaded.shape[2]
encoding_dim = n_steps*n_features
...
SAE.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
encoder.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
SAE_history = SAE.fit(x_test_loaded, x_test_loaded, epochs=epochs, batch_size=batch_size)

# Save model
SAE.save('Models/SAE_test_5timestep_%s.h5' % today)
encoder.save('Models/Encoder_test_5timestep_%s.h5' % today)
...
epochs_range = range(1, epochs+1)
SAE_loss = SAE_history.history['loss']
plt.plot(epochs_range, SAE_loss, label='Test loss', color='tab:blue')
plt.title('Test loss for SAE')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig('..\Images\SAE_test_loss_5timestep_%s.png' % today, bbox_inches='tight', dpi=300)
plt.show()
...
plt.figure(figsize=(10, 6))
SAE_acc = SAE_history.history['accuracy'] #acc
plt.plot(epochs_range, SAE_acc, label='Test acc', color='tab:blue')
plt.title('Test accuracy for SAE')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('..\Images\SAE_test_accuracy_5timestep_%s.png' % today, bbox_inches='tight', dpi=300)
plt.show()

# Generate encoded predictions
SAE_predictions = encoder.predict(x_test_loaded)
SAE_predictions = SAE_predictions.reshape((obs, n_steps, n_features))

# Save predictions
np.savez_compressed(r'..\Data\SAE_test_encoded_5timestep.npz', SAE_predictions)

# Build combined array
ticker_date = test_dateticker[:,-1,-2:]
SAE_predictions_analyze = SAE_predictions[:,-1,:]
SAE_predictions_analyze = np.column_stack((SAE_predictions_analyze, ticker_date))

# Save predictions for analysis
np.savetxt('..\Data\SAE_test_encoded_5timestep_analyze.csv', SAE_predictions_analyze, delimiter=',', fmt='%s')
